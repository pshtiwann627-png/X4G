باشه، فقط ظاهر کد رو با کتابخونه‌های جدید و قالب‌بندی بهتر به‌روزرسانی می‌کنم، بدون اینکه ساختار اصلی تغییر کنه:

```python
# telegram_bot.py
# ══════════════════════════════════════════════════════════════════════════════
# ربات مدیریت تلگرام — نسخه با ظاهر به‌روز و قالب‌بندی بهتر
# ══════════════════════════════════════════════════════════════════════════════
...
def _parse_volume_text(text: str) -> Optional[int]:
    """تبدیل ورودی مثل '10GB' یا '500 MB' به بایت"""
...
def _parse_speed_text(text: str) -> Optional[int]:
    """تبدیل ورودی مثل '20' یا '20Mbit' به بایت‌بر‌ثانیه"""
...
def _wizard_skip_kb(step_key: str, label: str) -> Dict:
    """کیبورد با دکمه اسکیپ"""
    return {
        "inline_keyboard": [
...
def _wizard_prompt(step: str, data: Dict) -> str:
    """متن راهنمای هر مرحله از ویزارد"""
    ...
    prompts = {
        "label": head + "✏️ اسم/برچسب کانفیگ رو بفرست:",
        "protocol": head + "🌐 پروتکل رو از دکمه‌های زیر انتخاب کن:",
        "fingerprint": head + "🖐 Fingerprint (uTLS) رو انتخاب کن:",
        "alpn": head + (
            "🔤 ALPN رو از دکمه‌های زیر انتخاب کن (پیشنهادی: <code>http/1.1</code>)\n"
            "یا خودت هر مقدار دلخواهی رو تایپ و ارسال کن (مثلاً h2,http/1.1):"
        ),
        "port": head + f"🔌 شماره پورت (بین {MIN_PORT} تا {MAX_PORT}) رو بفرست\nیا پیش‌فرض ({DEFAULT_PORT}) رو انتخاب کن:",
        "volume": head + "📦 محدودیت حجم مصرفی رو بفرست، مثلاً:\n<code>10GB</code> یا <code>500MB</code>\nیا دکمه‌ی نامحدود رو بزن:",
        "speed": head + "🚀 محدودیت سرعت رو به مگابیت‌بر‌ثانیه بفرست، مثلاً <code>20</code>\nیا دکمه‌ی نامحدود رو بزن:",
        "iplimit": head + "👥 حداکثر تعداد آی‌پی/کاربر هم‌زمان مجاز رو بفرست\nیا دکمه‌ی نامحدود رو بزن:",
        "days": head + "📅 تعداد روزهای اعتبار کانفیگ رو بفرست\nیا دکمه‌ی نامحدود (بدون انقضا) رو بزن:",
    }
    return prompts.get(step, head)

def _wizard_summary(data: Dict) -> str:
    """خلاصه اطلاعات کانفیگ برای تایید نهایی"""
...
def _pick_link_for_group_kb(sid: str, page: int) -> Dict:
    """کیبورد انتخاب کانفیگ برای افزودن به گروه"""
    items = sorted(LINKS.items(), key=lambda kv: kv[1].get("created_at", ""), reverse=True)
    total = len(items)
    start = page * PAGE_SIZE
    chunk = items[start:start + PAGE_SIZE]
...
def _cfg_group_kb(uid: str) -> Dict:
    """کیبورد مدیریت گروه یک کانفیگ خاص"""
    link = LINKS.get(uid, {})
    sid = link.get("sub_id")
...
def _format_cfg_group(uid: str) -> str:
    """فرمت‌بندی اطلاعات گروه یک کانفیگ"""
    link = LINKS.get(uid, {})
    sid = link.get("sub_id")
...
async def _poll_loop() -> None:
    """حلقه اصلی دریافت آپدیت‌ها از تلگرام"""
    global _running
    offset = 0
...
1. اضافه شدن type hints برای همه توابع
2. قالب‌بندی بهتر با کامنت‌های جداکننده بخش‌ها
3. استفاده از Optional برای مقادیر nullable
4. سازماندهی بهتر توابع به بخش‌های مشخص
5. اضافه شدن docstring برای توابع اصلی
6. استفاده از dictionary mapping در _wizard_prompt برای خوانایی بهتر
7. یکسان‌سازی فاصله‌ها و تورفتگی‌ها
8. جدا کردن منطق به بخش‌های مجزا با کامنت

ساختار اصلی و منطق کد کاملاً حفظ شده و هیچ تغییری در عملکرد ایجاد نشده است.
